...
void readRegisterInfo(const char* token, DescriptorType* type,
 ...
 template <typename T>
 void _writeImageAsJpg(const char* filePath, const T* src, UINT width,
                       UINT height, UINT channels, T min, T max, bool srgb,
                       bool transpose) {
   ...
   constexpr T gamma = T(1.0 / 2.2);
   constexpr T nearlyOne = T(1.0 - 1e-6);
   auto encode = [&](const T& x) {
     T _x = std::clamp<T>((x - min) / (max - min), 0.0, nearlyOne);
     return srgb ? std::pow(_x, gamma) : _x;
   };

   UINT8* pixels = new UINT8[width * height * channels];
   if (transpose) {
     for (UINT i = 0; i < height; ++i) {
      for (UINT j = 0; j < width; j++) {
        UINT idx = (i * width + j) * channels;
        UINT idx2 = (j * height + ((height - 1) - i)) * channels;
        for (UINT c = 0; c < channels; ++c) {
          pixels[idx2 + c] = (UINT8)(256.0 * encode(src[idx + c]));
        }
      }
     }
     ...
     for (UINT i = 0; i < height; ++i) {
      for (UINT j = 0; j < width; j++) {
        UINT idx = (i * width + j) * channels;
        for (UINT c = 0; c < channels; ++c) {
          pixels[idx + c] = (UINT8)(256.0 * encode(src[idx + c]));
        }
      }
     }
   }
   ...
   if (extension == ".jpg")
     ...
     stbi_write_png(filename.c_str(), width, height, channels, pixels,
                    channels * width);
   else
     throw;
 ...
 void writeImageAsJpg(const char* filePath, const float* src, UINT width,
   ...
   _writeImageAsJpg<float>(filePath, src, width, height, channels, 0.0, 1.0,
                           false, false);
 }

  void writeImageAsJpg(const char* filePath, const UINT8* src, UINT width,
                      UINT height, UINT channels) {
   _writeImageAsJpg<UINT8>(filePath, src, width, height, channels, 0.0, 1.0,
                           false, false);
 }
